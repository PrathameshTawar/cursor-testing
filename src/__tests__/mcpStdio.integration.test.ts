...
import { makeTmpDir, removeTmpDir, writeFile } from "./helpers";
...
describe("MCP stdio integration", () => {
  ...
  beforeEach(async () => {
    root = await makeTmpDir("mcp-stdio-");
    await writeFile(path.join(root, "code.ts"), "export const needle = 1;\n");
...
class StdioClient {
  ...
  async start(): Promise<void> {
    this.child = spawn(process.execPath, [path.join(process.cwd(), "dist", "server.js")], {
